...
from theme_constants import WAIT_TIME_MEDIUM, WAIT_TIME_SHORT,WAIT_TIME_LONG ,click_element, wait_for_element
...
def Top_menu_addNewItem(wait):
    try:
        ...
        print(f"Error in Add New Item method in Top menu: {e}")
        raise
...
def topmenu_name(wait,variable_name1,variable_name2):
    try:
        ...
        print(f"Error in the entering topmenu Item menu nname and method: {e}")
        raise
...
def edit_header_color(wait):
    try:
        ...
        color = wait.until(EC.element_to_be_clickable((By.XPATH, "//div[@class='ant-color-picker-color-block-inner' and contains(@style, 'background: rgb(114, 46, 209);')]")))
        color.click()  # Added parentheses to actually call the click method
        sleep(WAIT_TIME_SHORT)
...
def additional_option2(wait,x,y,z):
    try:
        ...
        X_value = wait.until(EC.presence_of_element_located((By.XPATH, "(//input[@placeholder='X'])[2]")))
        X_value.send_keys(x)
        sleep(WAIT_TIME_SHORT)
        print("Text Shadow: X value is added")
        Y_value = wait.until(EC.presence_of_element_located((By.XPATH, "(//input[@placeholder='Y'])[2]")))
        Y_value.send_keys(y)
        sleep(WAIT_TIME_SHORT)
        print("Text Shadow:Y value is added")
        Blur = wait.until(EC.presence_of_element_located((By.XPATH, "(//input[@placeholder='Blur'])[1]")))
        Blur.send_keys(z)
        sleep(WAIT_TIME_SHORT)
