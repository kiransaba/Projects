...
def edit_column(self):
    try:
        hover_col = self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR,".position-heading.column-heading" )))
        ActionChains(self.driver).move_to_element(hover_col).perform()
        sleep(WAIT_TIME_SHORT)
        print("Edit column button s hovered")
        column=self.wait.until(EC.element_to_be_clickable((By.ID,"edit-column" )))
        column.click()
        sleep(WAIT_TIME_SHORT)
        print("Edit column screen is opened")
    except Exception as e:
        print(f"Error occurred on Clicking Edit Columm button method: {e}")
        raise
def select_module(wait):
    try:
        ...
        select_module = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[@class='variable-name'])[2]")))
        select_module.click()
        sleep(WAIT_TIME_MEDIUM)
